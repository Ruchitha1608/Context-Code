...
def _load_graph(root: Path) -> dict:
    p = root / ".contextkit" / "info_graph.json"
    if not p.exists():
        return {}
    try:
...
def format_report(root: Path, r: dict, color: bool = True) -> str:
    ...
    out = [
        "",
        h(f"Contextkit Audit — {root.name}"),
        sep,
        "",
        h("OVERVIEW"),
...
def write_audit_context(root: Path, r: dict) -> Path:
    ...
    out_path = root / ".contextkit" / "AUDIT_CONTEXT.md"
...
def run_audit(root: Path) -> dict:
    ...
    if not graph:
        print(f"No info_graph.json found in {root}/.contextkit/", file=sys.stderr)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Contextkit Vibe Code Auditor")
    ...
    print(f"\nContextkit Auditor — {root.name}", file=sys.stderr)
    r = run_audit(root)

    # Save JSON
    out_path = Path(args.out) if args.out else (root / ".contextkit" / "audit_report.json")
