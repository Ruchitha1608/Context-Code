...
# -- Delegate to contextkit.ps1 for non-Claude tools ----------------------------
$_otherTools = @("--opencode","--cursor","--gemini","--copilot","--codex")
foreach ($a in $args) {
    if ($a -in $_otherTools) {
        $DG = Join-Path $env:USERPROFILE ".contextkit"
        $GrapePs1 = Join-Path $DG "contextkit.ps1"
        # Force-download fresh contextkit.ps1 to ensure delegation works
        $GrapeR2 = "/contextkit.ps1"
        $GrapeGH = "https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main/bin/contextkit.ps1"
        try {
            $tmp = "$GrapePs1.tmp"
        ...
        if (-not (Test-Path $GrapePs1)) {
            $GrapePs1 = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Definition) "contextkit.ps1"
        }
        if (-not (Test-Path $GrapePs1)) {
            Write-Host "[ckc] ERROR: $a requires contextkit.ps1, which is missing."
            Write-Host "[ckc]   Run: irm https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main/install.ps1 | iex"
            exit 1
        }
        & $GrapePs1 @args
        exit $LASTEXITCODE
...
try {
    ...
    if ($remoteVer) {
        try {
            if ([version]$remoteVer -gt [version]$localVer) {
                ...
                $updatedScript = Join-Path $DG "ckc.ps1"
    ...
    if ($needsVenv) {
        ...
        if (-not $foundPy) {
            ...
            Write-Host "[$Tool] After installing, close and reopen your terminal, then run ckc again."
    ...
    # Kill any previous MCP server BEFORE the contextkit upgrade.
    ...
    # locked mcp-graph-server.exe (step 2), leaving contextkit half-uninstalled.
    ...
    if ((Invoke-NativeQuiet $Python @("-c", "import contextkit.graph_builder")) -eq 0) {
        ...
        # mcp-graph-server.exe, leaving contextkit importable but graph-builder.exe missing.
        if (Test-Path $grapeBuilderExe) {
            $grapeOk = $true
        } else {
            Write-Host "[$Tool] contextkit partially installed (graph-builder.exe missing) -- reinstalling..."
        }
    } elseif ((Invoke-NativeQuiet $Python @("-c", "import contextkit")) -eq 0) {
        # contextkit imports but graph_builder submodule is missing (broken sdist install)
        Write-Host "[$Tool] contextkit.graph_builder missing -- upgrading contextkit..."
    }
    if (-not $grapeOk) {
    ...
    # Remove conflicting dg.exe from Python Scripts (old contextkit installed it; renamed to dg-graph in 3.9.34+)
    try {
    ...
    Write-Host "[$Tool]   1. Wait 5 minutes and run ckc again"
    Write-Host "[$Tool]   2. Update Claude Code: npm install -g @anthropic-ai/claude-code"
    Write-Host "[$Tool]   3. Join Discord for help: "
    Write-Host ""
    ...
    if (Test-Path $Gitignore) { Ensure-Line $Gitignore ".contextkit/" }
...
Whenever you make a decision, identify a task, note a next step, fact, or blocker during a conversation, append it to ``.contextkit/context-store.json``.
...
**To append:** Read the file -> add the new entry to the array -> Write it back -> call ``graph_register_edit`` on ``.contextkit/context-store.json``.
...
"@
    ...
    Remove-ClaudeMcpSafe "contextkit"
    $mcpAddExit = Invoke-NativeQuiet "claude" @("mcp", "add", "--transport", "http", "contextkit", "http://127.0.0.1:$port/mcp")
    if ($mcpAddExit -ne 0) {
        $mcpAddExit = Invoke-NativeQuiet "claude" @("mcp", "add", "--transport", "sse", "contextkit", "http://127.0.0.1:$port/mcp")
    }
    if ($mcpAddExit -ne 0) {
        $mcpAddExit = Invoke-NativeQuiet "claude" @("mcp", "add", "contextkit", "--url", "http://127.0.0.1:$port/mcp")
    }
    if ($mcpAddExit -ne 0) {
        Stop-McpServer $pidFile $portFile
        ...
        Write-Host "[$Tool] 2. Wait 5 minutes and run ckc again."
...
try {
    ...
    if (`$out) { Write-Output `$out; Write-Error "[contextkit] Context loaded (port `$port)" }
} catch {
    Write-Error "[contextkit] MCP server not reachable on port `$port -- run dgc to restart"
}
...
($stopTemplate.Replace("__PROJECT__", $resolvedProject).Replace("__DATADIR__", $DG)) | Set-Content -Path $stopPs1 -Encoding UTF8
    ...
    $lbServer   = if ($env:DG_LICENSE_SERVER) { $env:DG_LICENSE_SERVER } else { "https://contextkit-license-production.up.railway.app" }
    ...
    if (-not (Test-Path $lbOptFile) -and [Environment]::UserInteractive) {
        Write-Host ""
        Write-Host "[$Tool] Want to appear on the ContextKit leaderboard?"
        Write-Host "[$Tool] It shows how many tokens you've used. Only your chosen name is shared."
        Write-Host "[$Tool] View: https://contextkit.dev/leaderboard"
        $lbAns = ""
        ...
        if ($lbAns -match '^[Yy]') {
            ...
            Write-Host "[$Tool] Skipped. Run 'ckc --leaderboard' anytime to opt in later."
        }
        Write-Host ""
    }
    ...
    try {
        ...
        try {
            ...
            if (Test-Path $historyFile) {
                ...
                if ($lastId) {
                    Write-Host ""
                    Write-Host "[$Tool] To resume this session with contextkit:"
    ...
    if ($claudeExit -ne 0 -and $claudeExit -ne 130 -and $claudeExit -ne -1073741510) {
        Send-CliError "Running Claude" "Claude exited with code $claudeExit in ckc.ps1"
    }
