@echo off
:: dg - stable Windows bootstrap for Codex CLI + contextkit
:: Keeps the entrypoint minimal and delegates launcher logic to PowerShell.

setlocal

set "DG=%USERPROFILE%.contextkit"
set "LOCAL_PS1=%DG%\ck.ps1"
set "LOCAL_CMD=%DG%\ck.cmd"
set "PENDING_CMD=%DG%\ck.cmd.new"
set "BOOTSTRAP_PS1=%TEMP%\contextkit_ck_bootstrap.ps1"
set "REMOTE_PS1=https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main/bin/ck.ps1"
set "REMOTE_PS1_R2=/ck.ps1"
...
if defined SCOOP (
  if exist "%SCOOP%\shims\ck.cmd" (
    set "INSTALL_METHOD=scoop"
    set "REPAIR_CMD=scoop update contextkit"
  )
) else (
  if exist "%USERPROFILE%\scoop\shims\ck.cmd" (
    set "INSTALL_METHOD=scoop"
    set "REPAIR_CMD=scoop update contextkit"
  )
)
...
echo [ck] Error: bootstrap unavailable and local launcher missing.
if /i "%INSTALL_METHOD%"=="scoop" (
  echo [ck] Scoop is installed, but the local launcher payload is missing.
)
echo [ck] Run this once to repair the installation:
echo [ck]   %REPAIR_CMD%
exit /b 1
