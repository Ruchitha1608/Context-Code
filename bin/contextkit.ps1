...
# For --claude and --codex, delegates to ckc.ps1 / ck.ps1.
#
# Usage:
#   contextkit [path] --claude    Claude Code  (default)
#   contextkit [path] --codex     OpenAI Codex
...
if ($Arg0 -in @("--help","-h","?","/?")) {
    Write-Host ""
    Write-Host "  contextkit - ContextKit AI tool launcher"
    Write-Host ""
    Write-Host "  Usage:"
    Write-Host "    contextkit [path] <tool> [options]"
    Write-Host ""
    Write-Host "  Tools:"
    Write-Host "    --claude    Claude Code   (shorthand: ckc [path])"
    Write-Host "    --codex     OpenAI Codex  (shorthand: ck  [path])"
    ...
    Write-Host ""
    Write-Host "  Examples:"
    Write-Host "    contextkit . --claude"
    Write-Host "    contextkit C:\my\project --cursor"
    Write-Host "    contextkit C:\my\project --gemini"
    Write-Host "    contextkit C:\my\project --opencode"
    Write-Host "    contextkit C:\my\project --copilot"
    Write-Host "    contextkit C:\my\project --claude --resume <session-id>"
    Write-Host "    ckc .                        # same as contextkit . --claude"
    Write-Host "    ck  .                        # same as contextkit . --codex"
    Write-Host ""
    exit 0
}
...
if ($Assistant -in @("claude","codex")) {
    $Ps1Name  = if ($Assistant -eq "claude") { "ckc.ps1" } else { "ck.ps1" }
...
$_BaseUrl  = "https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main"
$_R2       = ""
...
if ($_RemoteVer -and ($_LocalVer -eq "0" -or ([version]$_RemoteVer -gt [version]$_LocalVer))) {
    ...
    _dl "$_R2/contextkit.ps1"        "$_BaseUrl/bin/contextkit.ps1"        (Join-Path $DG "contextkit.ps1")
    _dl "$_R2/contextkit.cmd"        "$_BaseUrl/bin/contextkit.cmd"        (Join-Path $DG "contextkit.cmd")
    _dl "$_R2/ckc.ps1"              "$_BaseUrl/bin/ckc.ps1"              (Join-Path $DG "ckc.ps1")
    _dl "$_R2/ck.ps1"               "$_BaseUrl/bin/ck.ps1"               (Join-Path $DG "ck.ps1")
    _dl "$_R2/ckc.cmd"              "$_BaseUrl/bin/ckc.cmd"              (Join-Path $DG "ckc.cmd")
    _dl "$_R2/ck.cmd"               "$_BaseUrl/bin/ck.cmd"               (Join-Path $DG "ck.cmd")
    _dl "$_R2/contextkit_launch.sh" "$_BaseUrl/bin/contextkit_launch.sh" (Join-Path $DG "contextkit_launch.sh")
    try { $_RemoteVer | Set-Content -Path $_VerFile -Encoding UTF8 } catch {}
    Write-Host "[$Tool] Updated to $_RemoteVer. Restarting..."
    $_newScript = Join-Path $DG "contextkit.ps1"
...
# -- cursor / gemini: need the full pipeline - load shared helpers from ckc.ps1 -
# Pull ckc.ps1 functions by dot-sourcing (it is designed to be safe to source)
$DgcPs1 = Join-Path $DG "ckc.ps1"
if (-not (Test-Path $DgcPs1)) {
    $DgcPs1 = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Definition) "ckc.ps1"
}
if (-not (Test-Path $DgcPs1)) {
    Write-Host "[$Tool] Downloading ckc.ps1 for shared helpers..."
    $DgcPs1 = Join-Path $DG "ckc.ps1"
    try {
        Invoke-WebRequest "$BaseUrl/bin/ckc.ps1" -OutFile $DgcPs1 -UseBasicParsing -TimeoutSec 15
    } catch {
        Write-Host "[$Tool] ERROR: could not download ckc.ps1."
        Write-Host "[$Tool]   irm $BaseUrl/install.ps1 | iex"
        exit 1
    }
}
...
$DataDir = Join-Path $ProjectPath ".contextkit"
...
# -- Remove conflicting dg.exe if present (old contextkit installed it; renamed to dg-graph in 3.9.34+) --
try {
...
if (-not (Test-Path $GraphExe)) {
    # Find graph_builder.py from installed contextkit package
    $pkgDir = & $Python -c "import contextkit, os; print(os.path.dirname(contextkit.__file__))" 2>$null
    if ($pkgDir) {
...
if (-not (Test-Path $McpServer)) {
    # Try installed contextkit package location
    $pkgDir = & $Python -c "import contextkit, os; print(os.path.dirname(contextkit.__file__))" 2>$null
    if ($pkgDir) {
...
if ($Assistant -eq "cursor") {
    ...
    $mcpServers | Add-Member -NotePropertyName "contextkit" `
    ...
    Write-Host ""
    Write-Host "[$Tool] NOTE: activate contextkit in Cursor (one-time setup):"
    Write-Host "[$Tool]   Cursor Settings -> Tools & MCP -> enable 'contextkit'"
    Write-Host ""
...
if ($Assistant -eq "gemini") {
    ...
    $existing.mcpServers | Add-Member -NotePropertyName "contextkit" `
...
if ($Assistant -eq "opencode") {
    ...
    $ocMcp | Add-Member -NotePropertyName "contextkit" `
...
if ($Assistant -eq "copilot") {
    ...
    $vsConf.servers | Add-Member -NotePropertyName "contextkit" `
    ...
    Write-Host "[$Tool] MCP URL: http://127.0.0.1:$McpPort/mcp"
    Write-Host ""
    Write-Host "[$Tool] NOTE: enable contextkit in VS Code (one-time setup):"
    Write-Host "[$Tool]   Copilot Chat panel -> Agent mode -> enable 'contextkit'"
    Write-Host ""
