#!/usr/bin/env bash
# ContextKit one-time setup
# Usage: curl -sSL https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main/install.sh | bash

set -euo pipefail


INSTALL_DIR="$HOME/.contextkit"
...
echo ""
echo "[install] All prerequisites satisfied. Installing contextkit..."
echo ""
...
import json, os
try:
    d = json.load(open(os.path.expanduser('~/.contextkit/identity.json')))
...
open(os.path.expanduser('$HOME/.contextkit/identity.json'), 'w').write(json.dumps(d))
" 2>/dev/null || true
...
URL_LAUNCH_R2="$R2/contextkit_launch.sh"
URL_LAUNCH_GH="$BASE_URL/bin/contextkit_launch.sh"

# ── Download core engine ──────────────────────────────────────────────────────
echo "[install] Downloading core engine..."
curl -fsSL "$URL_LAUNCH_R2" -o "$INSTALL_DIR/contextkit_launch.sh" 2>/dev/null || true
# Validate: if R2 returned an HTML error page, fall back to GitHub
if ! head -1 "$INSTALL_DIR/contextkit_launch.sh" 2>/dev/null | grep -q "^#"; then
  curl -fsSL "$URL_LAUNCH_GH" -o "$INSTALL_DIR/contextkit_launch.sh"
fi
chmod +x "$INSTALL_DIR/contextkit_launch.sh"
...
curl -fsSL "$BASE_URL/bin/contextkit" -o "$INSTALL_DIR/contextkit" && chmod +x "$INSTALL_DIR/contextkit"
...
if ! grep -q '.contextkit' "$SHELL_RC" 2>/dev/null; then
  echo 'export PATH="$PATH:$HOME/.contextkit"' >> "$SHELL_RC"
  echo "[install] Added ~/.contextkit to PATH in $SHELL_RC"
fi
...
echo "    contextkit /path/to/project --cursor   # Cursor IDE"
echo "    contextkit /path/to/project --gemini   # Gemini CLI"
echo ""
