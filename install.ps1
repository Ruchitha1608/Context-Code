...
try {
    # ContextKit one-time setup for Windows
    # Usage (PowerShell):
    ...
    $R2          = ""
    $BASE_URL    = "https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main"
    $INSTALL_DIR = "$env:USERPROFILE.contextkit"
    ...
    function Ensure-Venv {
        ...
        $probePyd = Get-ChildItem (Join-Path $venvDir "Lib\site-packages\contextkit") -Filter "*.pyd" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($probePyd) {
            $locked = $false
            try {
    ...
    Write-Host "  ContextKit Installer" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    if ($needsRestart) {
        ...
        Write-Host "  irm https://raw.githubusercontent.com/Ruchitha1608/Context-Code/main/install.ps1 | iex" -ForegroundColor White
        Write-Host "========================================" -ForegroundColor Cyan
        exit 0
    }
    ...
    Write-Host ""
    Write-Host "[install] All prerequisites satisfied. Installing contextkit..." -ForegroundColor Green
    Write-Host ""
    ...
    $launchDest = "$INSTALL_DIR\contextkit_launch.sh"
    if (Test-Path $launchDest) {
        Write-Host "[install] Core engine already present, skipping download."
    } else {
        try {
            Invoke-WebRequestWithRetry -Uri "$R2/contextkit_launch.sh" -OutFile $launchDest -Label "Download contextkit_launch.sh"
        } catch {
            Write-Host "[install] R2 unreachable, falling back to GitHub..."
            Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/contextkit_launch.sh" -OutFile $launchDest -Label "Download contextkit_launch.sh (GitHub fallback)"
        }
    }

    $step = "Downloading CLI wrappers"
    Write-Host "[install] Downloading CLI wrappers..."
    Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/ckc.cmd"       -OutFile "$INSTALL_DIR\ckc.cmd"       -Label "Download ckc.cmd"
    Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/ck.cmd"        -OutFile "$INSTALL_DIR\ck.cmd"        -Label "Download ck.cmd"
    # PS1 launchers: try R2 first (avoids GitHub CDN cache), fall back to GitHub
    try {
        Invoke-WebRequestWithRetry -Uri "$R2/ckc.ps1" -OutFile "$INSTALL_DIR\ckc.ps1" -Label "Download ckc.ps1"
    } catch {
        Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/ckc.ps1" -OutFile "$INSTALL_DIR\ckc.ps1" -Label "Download ckc.ps1 (GitHub fallback)"
    }
    try {
        Invoke-WebRequestWithRetry -Uri "$R2/ck.ps1" -OutFile "$INSTALL_DIR\ck.ps1" -Label "Download ck.ps1"
    } catch {
        Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/ck.ps1" -OutFile "$INSTALL_DIR\ck.ps1" -Label "Download ck.ps1 (GitHub fallback)"
    }
    Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/contextkit.cmd" -OutFile "$INSTALL_DIR\contextkit.cmd" -Label "Download contextkit.cmd"
    try {
        Invoke-WebRequestWithRetry -Uri "$R2/contextkit.ps1" -OutFile "$INSTALL_DIR\contextkit.ps1" -Label "Download contextkit.ps1"
    } catch {
        Invoke-WebRequestWithRetry -Uri "$BASE_URL/bin/contextkit.ps1" -OutFile "$INSTALL_DIR\contextkit.ps1" -Label "Download contextkit.ps1 (GitHub fallback)"
    }
    try {
    ...
    Invoke-Native { & $venvPy -m pip install "mcp>=1.3.0" uvicorn anyio starlette contextkit --quiet --constraint $constraintsFile } | Out-Null
    ...
    if ($userPath -notlike "*\.contextkit*") {
    ...
    Write-Host "    contextkit `"C:\path\to\your\project`" --cursor   # Cursor IDE" -ForegroundColor White
    Write-Host "    contextkit `"C:\path\to\your\project`" --gemini   # Gemini CLI" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "[install]   https://github.com/Ruchitha1608/Context-Code/issues/new" -ForegroundColor Yellow
    exit 1
}
